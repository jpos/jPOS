...
public class MUXPool extends QBeanSupport implements MUX, MUXPoolMBean {
    ...
    protected MUX firstAvailableMUX (long maxWait) {
        return firstAvailableMUX(Duration.ofMillis(maxWait).minus(Duration.between(Instant.EPOCH, Instant.now())));
    }
    protected MUX firstAvailableMUX (Duration maxWait) {
        Instant now = Instant.now();
        do {
            for (MUX m : mux)
                if (isUsable(m))
                    return m;
            ISOUtil.sleep (1000);
        } while (Duration.between(now, Instant.now()).toMillis() < maxWait.toMillis());
        return null;
    }
    protected MUX nextAvailableMUX (int mnumber, long maxWait) {
        return nextAvailableMUX(mnumber, Duration.ofMillis(maxWait).minus(Duration.between(Instant.EPOCH, Instant.now())));
    }
    protected MUX nextAvailableMUX (int mnumber, Duration maxWait) {
        Instant now = Instant.now();
        do {
            for (int i=0; i<mux.length; i++) {
