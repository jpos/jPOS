 ...
 *
 * Reads values configured inside {@code <enabled></enabled>} or
 * {@code <disabled></disabled>} to choose elements of which realm to log.
 * If {@code <enabled></enabled>} is defined, these realms ONLY will be logged. Disabled will not be taken into account.
 * If the {@code <disabled></disabled>} realms are defined, only this will not be shown, the rest will.
...
public class RealmLogFilter implements LogListener, XmlConfigurable, Configurable {
    /** Creates an empty filter; rules are wired by {@link #setConfiguration(Element)}. */
