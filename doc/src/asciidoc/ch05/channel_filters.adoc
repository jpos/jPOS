...
and sometimes multiplexers pools (MUXPool), a client calling `MUX.request(...)`
...
run link:http://beanshell.org[BeanShell] that can be modified on the fly. It's extremely
...
* `channel` - a reference to the ISOChannel associted with this filter
